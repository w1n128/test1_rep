...
(function () {
  ...
  function describeNetError(err) {
    ...
    if (t === 'timeout') return 'Не удалось открыть WebRTC/TURN-канал. Обновите обе страницы и попробуйте снова.';
  ...
  function startHosting() {
    ...
    const code = G.net.host({
      ...
      onPending: () => {
        lobbyStatus = 'Друг найден, строю WebRTC через TURN...';
      },
      onFallback: () => {
        lobbyStatus = 'TURN не открылся, пробую запасной relay...';
      },
      onConnect: () => {
  ...
  function startJoining() {
    ...
    G.net.join(joinCodeBuffer, {
      onReady: () => { lobbyStatus = 'Сигнальный канал открыт, жду ответа хоста...'; },
      onFallback: () => {
        lobbyStatus = 'TURN не открылся, жду запасной relay...';
      },
      onConnect: () => {
        G.net.send({ t: 'hello', v: G.net.PROTOCOL_VERSION });
  ...
  function drawPaused() {
    ...
    if (noviceMode) {
      ...
      ctx.fillStyle = '#a0e0ff';
      ctx.fillText('После 90 сек наступает ночь на 30 сек: динамика видна только в луче фонарика.', cx, baseY + 62);
      ctx.fillStyle = '#ffe060';
